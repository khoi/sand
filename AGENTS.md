...
swift run sand --config config.toml  # run CLI
```
...
- `fixtures/` holds sample configs, including `fixtures/sample_full_config.toml` for full schema coverage.
...
## Testing Guidelines
- Use Swift Testing assertions (`#expect`) and keep test inputs close to fixtures or inline TOML strings.
- When changing the config schema, update or add fixture coverage and the relevant config tests.

## Configuration & Operations
- Configuration is TOML loaded via `--config`; see `fixtures/*.toml` for example configurations
